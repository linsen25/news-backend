...
export class ArticlesService {
  ...
  async create(input: CreateArticleDto, actor: User): Promise<Article> {
    ...
    const article = await this.articles.create({
      ...
      coverFocalX: input.coverFocalX ?? 50,
      coverFocalY: input.coverFocalY ?? 50,
      isHeadline: this.permissions.has(actor, 'users.permissions.manage') ? input.isHeadline ?? false : false,
      homepagePriority: this.permissions.has(actor, 'users.permissions.manage') ? input.homepagePriority ?? 0 : 0,
  ...
  async update(
    ...
    const updated = await this.articles.update(id, {
      ...
      isHeadline: this.permissions.has(actor, 'users.permissions.manage') ? input.isHeadline : undefined,
      homepagePriority: this.permissions.has(actor, 'users.permissions.manage') ? input.homepagePriority : undefined,
      byline: input.byline?.trim(),
